            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className="mb-8 border-y border-border py-6">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{service.priceUnit}</p>
                </div>

                {/* CTA */}
                <div className="flex flex-col gap-4 sm:flex-row">
                  <AddToCartButton service={service} />
                  <Button asChild variant="outline" size="lg" className="gap-2 rounded-none bg-transparent px-8">
            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
              <div>
                ...
                <p className="text-base leading-relaxed text-muted-foreground">
                  Every {service.name.toLowerCase()} service includes our signature attention to detail and premium
                  cleaning products. Here's what you can expect.
                </p>
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                {service.includes.map((item, index) => (
                  <div key={index} className="flex items-start gap-3 bg-background p-4">
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {relatedServices.map((relatedService) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      <h3 className="mb-2 text-lg font-semibold text-foreground">{relatedService.name}</h3>
                      <p className="mb-4 text-sm text-muted-foreground line-clamp-2">
                        {relatedService.shortDescription}
                      </p>
                      <p className="font-semibold text-foreground">From {formatPrice(relatedService.startingPrice)}</p>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <section className="bg-foreground py-16 text-primary-foreground lg:py-24">
          <div className="mx-auto max-w-7xl px-6 text-center lg:px-8">
            ...
            <p className="mx-auto mb-8 max-w-2xl text-primary-foreground/70">
              Experience the Hygiapro difference. Professional service, premium results, and complete peace of mind.
            </p>
            <div className="flex flex-col justify-center gap-4 sm:flex-row">
              <Button asChild size="lg" variant="secondary" className="gap-2 rounded-none px-8">
                <Link href="/services">
