...
import { CheckCircle, ArrowRight, Calendar, Phone, Mail, Clock, MapPin, XCircle } from "lucide-react"
import { Header } from "@/components/layout/header"
import { Footer } from "@/components/layout/footer"
import { Button } from "@/components/ui/button"
import { verifyPaystackPayment } from "@/app/actions/paystack"
import { getBookingByReference } from "@/app/actions/bookings"
        ...
        <main className="flex flex-1 items-center justify-center px-6 pt-20">
          <div className="mx-auto max-w-lg text-center">
            ...
            <p className="mb-8 text-muted-foreground">
              We couldn't find your booking. Please check your reference number or contact support.
            </p>
            <Button asChild className="gap-2 rounded-none bg-accent text-accent-foreground hover:bg-accent/90">
              <Link href="/">
                Return Home
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-32">
        <div className="mx-auto max-w-lg text-center">
          ...
          <p className="mb-8 text-muted-foreground">
            Thank you for choosing Hygiapro, {bookingData.customerName.split(" ")[0]}! Your cleaning service has been
            booked successfully. We've sent a confirmation to {bookingData.customerEmail}.
          </p>
          ...
          <div className="mb-6 border border-accent bg-accent/10 p-4">
            ...
            <p className="mt-1 font-mono text-lg font-medium text-foreground">{reference}</p>
          </div>

          {/* Booking Summary */}
          <div className="mb-8 border border-border bg-card p-6 text-left">
            <h2 className="mb-4 font-semibold text-foreground">Booking Details</h2>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="font-medium text-foreground">
                    {format(new Date(bookingData.bookingDate), "EEEE, MMMM d, yyyy")}
                  </span>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="font-medium text-foreground">{formatTimeLabel(bookingData.bookingTime)}</span>
                </div>
              </div>
              <div className="flex items-start gap-3">
            ...
            <div className="mt-4 border-t border-border pt-4">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-accent">₦{bookingData.totalAmount.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 border border-border bg-secondary p-6 text-left">
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              <li className="flex items-start gap-3">
                <Calendar className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
                <span>Our team will arrive at your scheduled date and time</span>
              </li>
              <li className="flex items-start gap-3">
